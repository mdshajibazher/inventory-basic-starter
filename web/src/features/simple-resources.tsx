...
function renderTable<T extends Brand | Branch | Category | Unit | Tax | Currency | Warehouse | Account | ExpenseCategory>(
  ...
  if (resource === 'units') {
    return <Table headers={['Code', 'Unit', 'Group', 'Base', 'Operator', 'Value', 'Status', 'Action']}>{(items as Unit[]).map((item) => <tr key={item.id} className="border-t border-neutral-100"><td className="px-4 py-3">{item.unit_code}</td><td className="px-4 py-3 font-medium">{item.unit_name}</td><td className="px-4 py-3">{item.unit_group_title ?? item.unit_group?.title ?? '-'}</td><td className="px-4 py-3">{item.base_unit_name ?? item.base?.unit_name ?? 'Base unit'}</td><td className="px-4 py-3">{item.operator ?? '-'}</td><td className="px-4 py-3">{item.operation_value ?? '-'}</td><td className="px-4 py-3"><StatusBadge active={item.is_active} /></td><td className="whitespace-nowrap px-4 py-3"><div className="flex items-center justify-end gap-1">{actions.canDelete && item.can_delete !== false ? <ActionButton icon={Trash2} text="Delete" color="text-red-500 hover:text-red-600" bgColor="bg-red-50 hover:border-red-100 hover:bg-red-100" disabled={actions.saving} onClick={() => void actions.remove(item as T)} /> : <span className="text-xs text-neutral-500">{item.can_delete === false ? 'In use' : '-'}</span>}</div></td></tr>)}</Table>;
  }
  if (resource === 'taxes') {
