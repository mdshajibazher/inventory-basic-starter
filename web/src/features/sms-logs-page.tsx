...
export function SmsLogsPage() {
  ...
  return (
    <div>
      <PageHeader title="SMS Logs" subtitle={`${logs.length} shown from ${pagination?.total ?? logs.length}`} />
      ...
      {logs.length ? (
            ...
            <tbody>
              {logs.map((log) => (
                <tr key={log.id} className="border-t border-neutral-100 align-top">
                  ...
                  <td className="whitespace-nowrap px-4 py-3">{log.phone_number}</td>
        ...
        <div className="rounded-lg border border-dashed border-neutral-200 bg-white p-8 text-center text-sm text-neutral-500">
          {loading ? 'Loading SMS logs...' : 'No SMS logs found.'}
        </div>
      )}
