...
import type { EmailLog, PaginationMeta } from '@/src/types';
  ...
  return (
    <Screen contentStyle={styles.screen}>
      <View style={styles.header}>
        <View>
          <Text variant="headlineSmall">Email Logs</Text>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <DataTable style={styles.table}>
          <DataTable.Header>
            ...
            <DataTable.Title style={styles.subjectColumn}>Subject</DataTable.Title>
          ...
          {logs.map((log) => (
            <DataTable.Row key={log.id}>
              ...
              <DataTable.Cell style={styles.emailColumn}>{log.email}</DataTable.Cell>
              ...
              <DataTable.Cell style={styles.subjectColumn}>{log.subject}</DataTable.Cell>
      ...
      {!loading && logs.length === 0 ? <Text variant="bodyMedium" style={styles.empty}>No email logs found.</Text> : null}
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  ...
  table: { minWidth: 1280, borderRadius: 8, overflow: 'hidden', backgroundColor: '#ffffff' },
  dateColumn: { flex: 1.1 },
  userColumn: { flex: 1 },
  emailColumn: { flex: 1.5 },
  statusColumn: { flex: 0.8 },
  recordColumn: { flex: 1.1 },
  subjectColumn: { flex: 1.5 },
