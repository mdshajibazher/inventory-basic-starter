...
class StorePurchaseReturnRequest extends FormRequest
{
    ...
    {
        $returnId = $this->route('returnPurchase')?->id ?? $this->route('returnPurchase');

        return [
            'reference_no' => ['required', 'string', 'max:191', Rule::unique('return_purchases', 'reference_no')->ignore($returnId)],
    ...
    protected function prepareForValidation(): void
    {
        ...
        $this->merge([
            ...
            'return_note' => $this->input('return_note', $this->input('note')),
        ]);
    }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            foreach (['qty', 'purchase_unit', 'net_unit_cost', 'discount', 'tax', 'subtotal'] as $field) {
            ...
            foreach (['product_code', 'variant_id', 'product_batch_id', 'batch_no', 'tax_rate'] as $field) {
                if (! $this->has($field)) {
                    continue;
                }
            ...
            foreach ($productIds as $index => $productId) {
                if (in_array((int) $productId, $variantProductIds, true)) {
                    ...
                    } elseif (! blank($variantId) && ! ProductVariant::query()->where('product_id', $productId)->where('variant_id', $variantId)->exists()) {
